...
export function MedDuties() {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-lg border border-ink-200 dark:border-ink-800 bg-white dark:bg-ink-900 p-4 space-y-3">
      ...
      {assignments.length > 0 && (
        <>
          <section className="rounded-lg border border-ink-200 dark:border-ink-800 bg-white dark:bg-ink-900 p-4 space-y-3">
            ...
            {stats.spread > 3 && (
              <div className="rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-sm text-amber-900 flex items-center justify-between gap-2 flex-wrap">
                <span>
              ...
              {DOCS.map((d) => {
                ...
                return (
                  <li key={d.id} className="flex items-center gap-3">
                    ...
                    <span className="tabular-nums text-xs text-ink-600 dark:text-ink-300 w-10 text-right">
                      {c}
                    </span>
                  </li>
                );
            ...
            <div className="pt-1 flex gap-2 flex-wrap">
              ...
              <details className="text-sm">
                <summary className="cursor-pointer inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md border border-ink-300 dark:border-ink-700 bg-white dark:bg-ink-900 hover:bg-ink-50 dark:hover:bg-ink-800">
          ...
          <section className="rounded-lg border border-ink-200 dark:border-ink-800 bg-white dark:bg-ink-900 p-4 overflow-x-auto">
      ...
      <section className="rounded-lg border border-ink-200 dark:border-ink-800 bg-white dark:bg-ink-900 p-4 space-y-2">
        <h2 className="font-semibold text-ink-800 dark:text-ink-100 text-sm">
          ...
          {!llmReady && (
            <span className="ml-2 text-xs text-ink-500 dark:text-ink-400 font-normal">
              (regex only — add a key in Settings for full natural language)
            </span>
          )}
        </h2>
        <form
