...
interface FhirObservation {
  readonly code?: { readonly text?: string; readonly coding?: ReadonlyArray<{ readonly code?: string; readonly display?: string }> };
...
export function PostVisit() {
  ...
  const askAboutRec = useCallback(
    async (recIdx: number) => {
      ...
      const cacheNs = "meduni-postvisit-chat-v2";
      ...
      if (!llmReady) {
        ...
        setChatByRec((m) => ({
          ...m,
          [recIdx]: [...withQ, { id: withQ.length, from: "ai", text: reply }],
        }));
        setLoadingRec(null);
        return;
      }
      const grounding = `Visit on ${visit.date} with ${visit.doctor}. Diagnosis: ${visit.diagnosis}. The ONE recommendation the patient is asking about (#${recIdx + 1} of ${visit.recommendations.length}): "${rec}"`;
      const r = await callLLM({
        system: `You are a patient companion answering a question about ONE specific recommendation from the patient's last visit. Ground every claim in that single recommendation — quote or paraphrase it directly. If the question drifts to a different recommendation or something outside this record, say "that's something to ask your doctor at the next visit." Maximum 3 sentences.\n\n${grounding}`,
        messages: [{ role: "user", content: q }],
        maxTokens: 220,
        temperature: 0.3,
      });
      setLoadingRec(null);
      const replyText = r.ok ? r.text.trim() : scriptedReply(q);
      if (r.ok) cacheSet(cacheNs, cacheKey, replyText);
      setChatByRec((m) => ({
        ...m,
  ...
  return (
    <div className="space-y-6">
      <header className="flex items-start flex-wrap justify-between gap-3">
        <div>
          <h1 className="display text-3xl sm:text-4xl text-ink-900 dark:text-ink-100">
            <ShinyText text="PostVisit companion" speed={8} />
          </h1>
      ...
      {tab === "recap" && (
        <section className="space-y-4">
          <article className="rounded-lg border border-ink-200 dark:border-ink-800 bg-white dark:bg-ink-900 p-5 space-y-3">
            <div className="flex items-start flex-wrap justify-between gap-2">
              ...
              <span className="pv-diagnosis-chip text-xs bg-emerald-100 text-emerald-800 border border-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-300 dark:border-emerald-400/40 px-2 py-0.5 rounded-md">
                {visit.diagnosis}
              </span>
            </div>
            <div className="space-y-2">
              ...
              {!llmReady && (
                <p className="text-xs text-amber-800 bg-amber-50 border border-amber-200 rounded-md px-2 py-1">
                  Demo fallback — answers are scripted. Add a key in Settings for grounded LLM replies.
                </p>
              )}
              <ol id="postvisit-recs" className="space-y-1.5">
                ...
                {visit.recommendations.map((r, idx) => {
                  ...
                  return (
                    <li
                      key={`${visit.id}-${idx}`}
                      className="rounded-md border border-emerald-200 dark:border-emerald-900/50 bg-gradient-to-r from-emerald-50/70 to-cream-50 dark:from-emerald-950/30 dark:to-ink-900 overflow-hidden"
                    >
                      <button
                        type="button"
                      ...
                      >
                        <span className="text-xs tabular-nums text-emerald-700/70 dark:text-emerald-300/70 mt-0.5 font-semibold">
                          {String(idx + 1).padStart(2, "0")}
                        </span>
                        <span className="flex-1 text-sm text-ink-800 dark:text-ink-100">
                          {r}
                        </span>
                        <span className="text-xs text-emerald-700 dark:text-emerald-300 shrink-0 mt-0.5">
                          {isOpen
                            ? "Close"
                            : chat.length > 0
                              ? `${(chat.length / 2) | 0} Q&A`
                      ...
                      {isOpen && (
                        <div className="border-t border-emerald-200/60 dark:border-emerald-900/50 px-3 py-2 space-y-2 bg-white dark:bg-ink-900">
                          {chat.length === 0 && (
                            <p className="text-xs text-ink-500 dark:text-ink-400 italic">
                              Try: "why this one?" · "any side effects?" · "how long until it helps?"
                            </p>
                          )}
                          {chat.map((m) => (
                            <div
                          ...
                          {isLoading && (
                            <div className="text-xs text-ink-500 dark:text-ink-400 inline-flex items-center gap-2">
      ...
      {tab === "scribe" && (
        <section className="space-y-3 rounded-lg border border-ink-200 dark:border-ink-800 bg-white dark:bg-ink-900 p-5">
          ...
          <div className="text-sm rounded-md border border-ink-200 dark:border-ink-800 bg-ink-50 dark:bg-ink-800/60 p-3 min-h-[5rem]">
            {finalScribe || (
...
const SCRIBE_SECTIONS: ReadonlyArray<{ readonly key: keyof ScribeExtract; readonly label: string; readonly tone: string }> = [
...
function scriptedReply(q: string): string {
  const ql = q.toLowerCase();
  if (ql.includes("coffee"))
    return "Coffee is fine in moderation — 1 to 2 cups a day is reasonable. The visit recommendation focused on the Mediterranean diet rather than caffeine.";
  if (ql.includes("avoid") || ql.includes("food"))
    return "Focus on plants, olive oil, and fish. Reduce red meat, fried food, and salty snacks. The recommendation was a Mediterranean-style diet.";
  if (ql.includes("long") || ql.includes("better"))
    return "Amlodipine takes 1-2 weeks to reach full effect. If chest pain returns at lower exertion than before, call the office.";
  if (ql.includes("ankle") || ql.includes("swell"))
    return "Mild ankle swelling is a known amlodipine side effect. If it's only your feet and not your face or breathing, mention it at the next visit.";
  return "That's something to ask your doctor at the next visit.";
}
