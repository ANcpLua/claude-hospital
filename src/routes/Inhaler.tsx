...
import { Send, Sparkles, Shuffle, RefreshCw } from "lucide-react";
import {
  aqiColor,
  fetchAqi,
  isAqiKeyAvailable,
...
import { AustriaMap, type AustriaHospital } from "../components/AustriaMap";
...
export function Inhaler() {
  ...
  return (
    <div id="inhaler-route" className="space-y-6">
      <header className="flex items-start justify-between flex-wrap gap-3 border-b border-orange-200/60 dark:border-orange-900/50 pb-4">
      ...
      <section id="inhaler-aqi-card" className="rounded-lg border border-ink-200 dark:border-ink-800 bg-white dark:bg-ink-900 p-4 flex flex-wrap gap-3 items-end text-sm">
      ...
      {persona === "pulmonologist" && (
        <Cohort seeds={seeds} reading={reading} llmReady={llmReady} />
      )}
...
function GenZPatient({ reading }: { readonly reading: AqiReading }) {
  ...
  return (
    <section className="space-y-4">
      ...
      <article className="rounded-lg border border-ink-200 dark:border-ink-800 bg-white dark:bg-ink-900 p-4 space-y-2">
        ...
        <ul className="text-sm space-y-1.5">
          ...
          <li>
            <strong>When to call:</strong> chest tight at rest, rescue inhaler used more than 2× in a day, or lips/fingers turning blue.
          </li>
        </ul>
      </article>

      <article className="rounded-lg border border-ink-200 dark:border-ink-800 bg-cream-50 dark:bg-ink-950 p-4 text-xs text-ink-600 dark:text-ink-300">
...
function Cohort({
  ...
  const ordered = useMemo(() => {
    ...
    switch (sortBy) {
      case "aqi": return withAqi.sort((a, b) => b.aqi - a.aqi);
      case "fev1": return withAqi.sort((a, b) => parseInt(a.p.lastFev1) - parseInt(b.p.lastFev1));
      default: return withAqi.sort((a, b) => RISK_ORDER[a.p.risk] - RISK_ORDER[b.p.risk]);
    }
    ...
    <section className="rounded-lg border border-ink-200 dark:border-ink-800 bg-white dark:bg-ink-900 p-5 space-y-4">
      ...
      {selectedHospital && trend ? (
        ...
        >
          ...
          <div className="rounded-md bg-white/60 dark:bg-ink-950/60 p-2 border border-white/80 dark:border-ink-800">
        ...
        <div className="rounded-md border border-dashed border-ink-300 dark:border-ink-700 bg-cream-50/60 dark:bg-ink-950/60 p-4 text-center text-sm text-ink-500 dark:text-ink-400">
...
function advisoryTextFor(aqi: number): string {
  ...
  if (aqi > 100) return "Unhealthy for sensitive groups. Issue an advisory to asthma / COPD patients in this bundesland.";
