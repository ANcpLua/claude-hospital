...
// Per request: origin allowlist → per-IP sliding window → body parse →
// Turnstile siteverify → daily cap (consumed only on accepted requests) →
// forward to Gemini with the server-held key.
//
// Env:
...
const IP_WINDOW_MS = Number(process.env.IP_WINDOW_MINUTES ?? 60) * 60 * 1000;
...
function checkIp(ip: string): boolean {
    ...
    arr.push(now);
    if (arr.length === 0) ipLog.delete(ip);
    else ipLog.set(ip, arr);
    return true;
}
